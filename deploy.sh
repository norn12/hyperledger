...
echo -e "${GREEN}=== 2. Joining Hospital Peer0 to Channel ===${NC}"
...
echo -e "${GREEN}=== 3. Joining Insurer Peer0 to Channel ===${NC}"
...
echo -e "${GREEN}=== 5. Installing Chaincode on both Orgs ===${NC}"
# Insurer Peer 0
...
echo -e "${GREEN}=== 6. Approving for both Orgs ===${NC}"
...
# Hospital Approve
peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer1.zerotrust.com --tls --cafile $ORDERER_CA --channelID healthchannel --name health --version 1.0 --package-id $FULL_PACKAGE_ID --sequence 1 --signature-policy "OR('HospitalMSP.member')"
...
peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer1.zerotrust.com --tls --cafile $ORDERER_CA --channelID healthchannel --name health --version 1.0 --sequence 1 --signature-policy "OR('HospitalMSP.member')" \
...
echo -e "${GREEN}✓ ZeroTrustBlock Chaincode is Active!${NC}"
