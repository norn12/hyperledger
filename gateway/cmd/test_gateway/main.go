...
func main() {
	...
	recordID, metrics, err := gw.WriteHealthRecord("PATIENT_XYZ", data, "ipfs://CID_123", "DIAGNOSIS", "ZKP_HASH_BEEF")
	if err != nil {
		log.Fatalf("Write record failed: %v", err)
	}
	fmt.Printf("✓ Transaction success! Latency: %dms\n", metrics.LatencyMs)

	// Test: Read record back
	fmt.Println("Retrieving record through Zero-Trust access control...")
	record, _, err := gw.ReadHealthRecord(recordID, "HOSPITAL_A", "ZKP_HASH_BEEF")
	if err != nil {
